...
function App() {
  ...
  const handleMouseMove = (e) => {
    if (isWarping || showProjects || window.innerWidth < 768) return;
    const factor = 25; 
    ...
    <div className="main-container" onMouseMove={handleMouseMove} onMouseLeave={() => setTilt({ x: 0, y: 0 })}>
      ...
      {!showProjects && !showRocket && (
        <div
          ...
          style={{ transform: window.innerWidth > 768 ? `perspective(1000px) rotateX(${tilt.x}deg) rotateY(${tilt.y}deg)` : 'none' }}
        >
          ...
          <div className="discord-card" onClick={e => e.stopPropagation()}>
            ...
            <div className="discord-pfp-wrapper">
              <div className="avatar-decoration-container">
                <img src={`https://dcdn.dstn.to/avatars/${DISCORD_ID}?size=128`} className="discord-mini-pfp" alt="" />
                <img src={`https://dcdn.dstn.to/avatar-decoration/${DISCORD_ID}`} className="avatar-decoration-overlay" onError={(e) => e.target.style.display = 'none'} alt="" />
              </div>
              <div className="discord-status-dot" style={{ backgroundColor: getStatusColor(status?.discord_status) }} />
            </div>
            <div className="discord-body">
              <div className="discord-badge-container"><img src={`https://dcdn.dstn.to/badges/${DISCORD_ID}`} className="real-badges" alt="" /></div>
                      ...
                      <img 
                        src={act.assets?.large_image 
                            ? (act.assets.large_image.startsWith('mp:external') 
                                ? act.assets.large_image.replace(/mp:external\/.*?\/(https?)\//, '$1://')
                                : `https://cdn.discordapp.com/app-assets/${act.application_id}/${act.assets.large_image}.png`)
                            : `https://dcdn.dstn.to/app-icons/${act.application_id}`} 
                        className="act-img" 
                        alt="" 
      ...
      <div className="audio-player-bar">
        ...
        <div className="audio-center">
          <div className="audio-controls">
            ...
            <button className="play-pause-btn" onClick={togglePlay}>{isPlaying ? <svg viewBox="0 0 24 24" fill="currentColor" width="28"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z" /></svg> : <svg viewBox="0 0 24 24" fill="currentColor" width="28"><path d="M8 5v14l11-7z" /></svg>}</button>
